...
### Trace-id http format

The [`trace-id` field](#trace-id) represents the ID of the whole trace forest.
It is represented as a 16-bytes array and represented as 32 lower case HEX
digits. For example, `4bf92f3577b34da6a3ce929d0e0e4736`.

Implementations HAVE TO ignore the `traceparent` when the `trace-id` is invalid.
For instance, if it contains non-HEX characters or all zeroes.

### Parent-id http format

The [`parent-id` field](#parent-id) represents the ID of this call as known by
the caller. It is represented as 8 bytes array and represented as 16 lower case
HEX digits. For example, `00f067aa0ba902b7`.

Implementations HAVE TO ignore the `traceparent` when the `trace-id` is invalid.
For instance, if it contains non-HEX characters or all zeroes.

### Trace-flags http format
...
When [`recorderd` flag](#recorded-flag) set, the least significant bit documents
that the caller may have recorded trace data. A caller who does not record trace
