...
2. Because the `traceparent` header is not received, the vendor creates a new `trace-id` and `parent-id` that represents the current request. (Note: Even if deciding to not sample the request, generating IDs is important to be able to propagate that sampling decision downstream via the sampled flag.)
