...
1. **Update key value**. The value of any key can be updated. Modified key SHOULD be moved to the beginning of the list. This is the most common mutation.
2. **Delete the key-value pair**. Any key-value pair MAY be deleted. It is highly discouraged to delete keys that wasn't generated by the same tracing system or platform. Deletion of unknown key-value pairs will break correlation in other system.
3. **Make key-value pair first**. Tracing system MAY move any key-value pair to the first position. This mutation will indicate to the downstream which tracing system or platform sent this request.
4. **Add new key-value pair**. New key-value pair should be added into the beginning of the list.